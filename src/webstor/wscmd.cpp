...
static StringWithLen s_cmdFlags =
    ...
    { STRING_WITH_LEN( "-i -s -H -U -P -G -a -f -n -p -M -T -m -x -d -z -b -v -help --help -? --? -W -k" ) };
#else
    { STRING_WITH_LEN( "-i -s -H -U -P -G -a -f -n -p -M -T -m -x -d -z -b -v -help --help -? --?" ) };
#endif
...
usage()
{
    std::cout <<
        ...
        "    -M md5sum of object (for 'put'),                                           \n"
        "    -T stor-type (S3, GCS, SWIFT or WALRUS; default: S3)                       \n"
        ...
        "    WS_STOR_TYPE   - instead of option '-T',                                   \n"
...
readEnvVars( Options *options )
{
    ...
    readEnvVar ("WS_STOR_TYPE", &options->storType );
...
parseCommandLine( int argc, char **argv, Options *options )
{
    ...
    for( int i = 1; i < argc; i++ )
    {
        ...
        if( tryGetValue( "-i", &i, argc, argv, &options->accKey ) ||
            ...
            tryGetValue( "-M", &i, argc, argv, &options->md5String ) ||
            tryGetValue( "-T", &i, argc, argv, &options->storType ) ||
...
static boost::optional<MD5Digest>
md5_digest_from_string(const std::string& s)
{
    if (s.empty())
    {
        return boost::none;
    ...
    else
    {
        ...
        {
            const std::string b(s.data() + 2 * i, 2);
            digest[i] = boost::lexical_cast<MD5Digest::value_type>(b);
        }
...
put( WsConnection *conn, StreamUploader *uploader, const Options &options )
{
    ...
    const boost::optional<MD5Digest> digest(md5_digest_from_string(options.md5String));

    WsPutResponse response;
    conn->put( options.bucketName.c_str(),
               options.prefix.c_str(),
               uploader,
               uploader->totalSize(),
               digest ? &(*digest) : nullptr,
               nullptr /* contentType */,
               conn->c_noCacheControl,
               options.makePublic,
               false /* useSrvEncrypt */,
               &response );
...
execute( const Options &options, Statistics *stat )
{
    ...
    // Infer storage type from the host name, unless explicitly configured.

    WsStorType storType = WsStorType::WST_S3;

    if( options.storType.empty() )
    {
        if ( !options.host.empty() )
        {
            if( strstr( options.host.c_str(), ".amazonaws.com" ) )
                storType = WsStorType::WST_S3;
            else if( strstr( options.host.c_str(), ".googleapis.com" ) )
                storType = WsStorType::WST_GCS;
            else
                storType = WsStorType::WST_WALRUS;
        }
    }
    else
    {
        storType = boost::lexical_cast<WsStorType>(options.storType);
    }
    ...
    config.isHttps = storType != WsStorType::WST_WALRUS && options.isHttps;
