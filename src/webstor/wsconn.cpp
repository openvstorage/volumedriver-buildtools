    ...
    std::string *signature )
{
    ...
    if( storType == WsStorType::WST_WALRUS )
...
static std::string
md5_as_base64_string(const MD5Digest& digest)
{
    std::string encoded;
    append64Encoded(&encoded, digest.data(), digest.size());
    return encoded;

    // //    CryptoPP::Base64Encoder enc;
    // std::string res;
    // //    enc.Attach(new CryptoPP::StringSink(res));
    // //    enc.PutMessageEnd(digest.data(), digest.size());

    // // For some reason yet unknown to be the encoded string has a trailing newline -
    // // strip it off.
    // BIO & bio, *b64;
...
static void
setRequestHeaders( const std::string &accKey,
                   const std::string &secKey,
                   const MD5Digest* md5_digest,
                   const char *contentType,
                   unsigned cacheMaxAge,
                   bool makePublic,
                   bool srvEncrypt,
                   const char *action,
                   ...
                   ScopedCurlList *plist )
{
    ...
    const std::string md5_string(md5_digest ? md5_as_base64_string(*md5_digest) : "");

    calcSignature( accKey,
                   secKey,
                   md5_digest ? md5_string.c_str() : nullptr,
                   contentType,
                   date,
                   makePublic,
                   srvEncrypt,
    ...
    appendRequestHeader( "Content-MD5",
                         md5_digest ? md5_string.c_str() : nullptr,
                         plist );
...
Request::saveIfCurlError( CURLcode curlCode )
{
    ...
    if( curlCode != CURLE_OK && curlCode != CURLE_WRITE_ERROR )
    {
        ...
        if( *m_curlErrorDetails )
        {
            ...
            m_error.reset( new WsCurlException( errCurl, m_curlErrorDetails ) );
        }
        else
        {
            m_error.reset( new WsCurlException( errCurl, curl_easy_strerror( curlCode ) ) );
        }
...
Request::saveError()
{
    ...
    try
    {
        ...
        {
            m_error.reset( new WsStdException( e.what() ) );
        }
        catch( ... )
        {
            m_error.reset( new WsUnknownException( errUnexpected ) );
        }
    }
    catch( const std::bad_alloc & )
    {
...
protected:
    ...
    virtual WsPutRequestUploader::SeekResult onSeek( off_t off,
                                                     WsPutRequestUploader::SeekWhence w )
    {
        return WsPutRequestUploader::SeekResult::CannotSeek;
    }
...
protected:
    void            throwXmlParserError() { throw WsXmlParserException( errParser ); }
...
private:
    ...
    static int      handleSeek(void *ctx, curl_off_t offset, int origin);
...
WsRequest::onPrepare( CURL *curl )
{
    ...
    curl_easy_setopt_checked( curl, CURLOPT_SEEKFUNCTION, handleSeek );
    curl_easy_setopt_checked( curl, CURLOPT_SEEKDATA, this );
}
...
int
WsRequest::handleSeek( void* ctx, curl_off_t off, int origin )
{
    WsRequest *req = static_cast< WsRequest *>( ctx );
    return req->handleSeek_(off, origin);
}
    ...
    size_t count, size_t memberSize ) // nofail
{
    try
    {
        ...
        if( startsWith( p, size, STRING_WITH_LEN( "HTTP" ), &prefixLen ) )
        {
            ...
            if( startsWith( p, size, STRING_WITH_LEN( "200" ) ) ||
                startsWith( p, size, STRING_WITH_LEN( "204" ) ) )
            {
                m_responseDetails.status = WS_RESPONSE_STATUS_SUCCESS;
            }
            else if( startsWith( p, size, STRING_WITH_LEN( "404" ) ) )
            {
            ...
            else if( startsWith( p, size, STRING_WITH_LEN( "301" ) ) ||
                startsWith( p, size, STRING_WITH_LEN( "400" ) ) ||
                startsWith( p, size, STRING_WITH_LEN( "403" ) ) ||
                startsWith( p, size, STRING_WITH_LEN( "409" ) ) ||
                startsWith( p, size, STRING_WITH_LEN( "500" ) ) ||
                startsWith( p, size, STRING_WITH_LEN( "503" ) ) ||
                startsWith( p, size, STRING_WITH_LEN( "503" ) ) )
            {
...
static WsPutRequestUploader::SeekWhence
int_to_whence_(int origin)
{
    switch (origin)
    {
    case SEEK_SET:
        return WsPutRequestUploader::SeekWhence::Set;
    case SEEK_CUR:
        return WsPutRequestUploader::SeekWhence::Cur;
    case SEEK_END:
        return WsPutRequestUploader::SeekWhence::End;
    default:
        throw std::runtime_error("invalid whence specified: " +
                                 boost::lexical_cast<std::string>(origin));
    }
}

int
WsRequest::handleSeek_( curl_off_t off, int origin )
{
    try
    {
        return onSeek(off, int_to_whence_(origin));
    }
    catch (...)
    {
        saveError();
        return WsPutRequestUploader::Fail;
    }
}
...
{
public:
    WsGetRequest( const char *name,
                  WsGetResponseLoader *loader,
                  WsStorType storType,
                  bool newest = false );
...
private:
    virtual size_t
    onLoadBinary( const void *chunkData,
                  size_t chunkSize,
                  size_t totalSizeHint );
    ...
    void
    setNewestHeader(WsStorType t, bool newest);

    WsGetResponseBufferLoader m_builtinLoader;
    WsGetResponseLoader *m_loader;
};

WsGetRequest::WsGetRequest( const char *name,
                            WsGetResponseLoader *loader,
                            WsStorType storType,
                            bool newest )
    : WsRequest( name )
    ...
    , m_loader( loader )
{
    setNewestHeader(storType, newest);
}
...
void
WsGetRequest::setNewestHeader(WsStorType t, bool newest)
{
    if (t == WsStorType::WST_SWIFT)
    {
        appendRequestHeader("X-Newest", newest ? "True" : "False", &headers);
    }
    else if (newest)
    {
        throw std::runtime_error("Requesting the latest version is only supported for SWIFT stores");
    }
}
...
{
public:
    WsPutRequest( const char *name,
                  WsPutRequestUploader *uploader,
                  size_t totalSize );
    ...
    void
    setUpload( const void *data, size_t size );

private:
    virtual size_t
    onUploadBinary( void *chunkBuf,
                    size_t chunkSize );

    virtual WsPutRequestUploader::SeekResult
    onSeek( off_t off,
            WsPutRequestUploader::SeekWhence w );
...
WsPutRequest::WsPutRequest( const char *name,
                            WsPutRequestUploader *uploader,
                            size_t totalSize )
    : WsRequest( name )
...
WsPutRequestUploader::SeekResult
WsPutRequest::onSeek( off_t off,
                      WsPutRequestUploader::SeekWhence whence )
{
    return m_uploader->onSeek( off, whence );
}
...
WsListObjectsRequest::onSetXmlValue( const char *value, int len )
{
    ...
    case WS_RESPONSE_NODE_PREFIX:
        ...
        {
            if( m_storType == WsStorType::WST_WALRUS )
            {
        ...
        else if( m_storType == WsStorType::WST_WALRUS )
        {
...
handleErrors( const WsResponseDetails &details )
{
    ...
    case WS_RESPONSE_STATUS_UNEXPECTED:
        {
            ...
            throw WsStatusUnexpectedException( errUnexpected );
        }

    case WS_RESPONSE_STATUS_HTTP_RESOURSE_NOT_FOUND:
        {
            throw WsStatusResourceNotFoundException( errHTTPResourceNotFound,
                                                     details.url.c_str() );
        }
    ...
    case WS_RESPONSE_STATUS_HTTP_OR_WS_FAILURE: // could not read more details from the payload, treat as http error.
        {
            throw WsStatusHttpFailureException( errHTTP, details.httpStatus.c_str() );
        }
    ...
    case WS_RESPONSE_STATUS_FAILURE_WITH_DETAILS:
        {
            ...
            throw WsStatusFailureWithDetailsException( errWsDetail,
...
throwSummary( const char *op, const char *key )
{
    ...
    {
        throw WsStdException( errOpSummary, op, key, e.what() );
    }
    catch( ... )
    {
        throw WsUnknownException( errOpSummary, op, key, errUnexpected );
    }
}
...
WsConnection::WsConnection( const WsConfig &config )
    ...
    , m_sslVerifyHost( config.sslVerifyHost )
    ...
    , m_connectTimeout( s_defaultConnectTimeout )
{
    ...
    m_baseUrl.append( config.host && *config.host ? config.host : ( config.storType == WsStorType::WST_GCS ? s_defaultGcsHost : s_defaultS3Host ) );

    const char* port = config.port;

    if( config.storType == WsStorType::WST_WALRUS && ( !port || !*port ) )
    {
    ...
    if( config.storType == WsStorType::WST_WALRUS )
    {
    ...
    if ( config.storType == WsStorType::WST_S3 && config.host && !strncmp( config.host, prefix.str, prefix.len ) )
    {
...
{
    CASSERT( static_cast<int>(c_maxWaitAny) == static_cast<int>(EventSync::c_maxEventCount) );
    dbgAssert( implies( count, cons ) );

    if( count > EventSync::c_maxEventCount )
    {
        throw WsTooManyConnectionsException( errTooManyConnetions );
    }
...
void
WsConnection::prepare( WsRequest& request,
                       const char *bucketName,
                       const char *key,
                       const char *contentType,
                       unsigned cacheMaxAge,
                       ...
                       const MD5Digest* md5_digest)
{
    ...
    if(  m_isHttps )
    {
        ...
        curl_easy_setopt_checked( m_curl,
    ...
    curl_easy_setopt_checked( m_curl,
                              CURLOPT_HTTPHEADER,
                              static_cast< curl_slist * >( request.headers ) );
...
void
WsConnection::init( WsRequest& request,
                    const char *bucketName,
                    const char *key,
                    const char *keySuffix,
                    const char *contentType,
...
WsConnection::createBucket( const char *bucketName,  bool makePublic )
{
    ...
    try
    {
        ...
        if( m_storType != WsStorType::WST_WALRUS && !m_region.empty() )
        {
            payload.reserve( 256 );
...
void
WsConnection::put( WsRequest& request,
                   const char *bucketName,
                   const char *key,
                   const MD5Digest* md5_digest,
                   const char *uploadId,
                   ...
                   WsPutResponse *response )
{
    ...
    init( request,
          bucketName,
          key,
          uploadId ? keySuffix.c_str() : NULL,
          contentType ? contentType : s_contentTypeBinary,
          cacheMaxAge,
          makePublic,
          useSrvEncrypt,
          md5_digest );
...
void
WsConnection::put( const char *bucketName,
                   const char *key,
                   const void *data,
                   size_t size,
                   const MD5Digest* md5_digest,
...
void
WsConnection::pendPut( AsyncMan *asyncMan,
                       const char *bucketName,
                       const char *key,
                       const void *data,
                       size_t size,
                       ...
                       bool useSrvEncrypt )
{
    ...
    try
    {
        ...
        std::unique_ptr< WsPutRequest > request( new WsPutRequest( key,
                                                                   data,
                                                                   size ) );
...
void
WsConnection::get( const char *bucketName,
                   const char *key,
                   ...
                   bool newest )
{
    ...
    LOG_TRACE( "enter get: conn=0x%llx, bucket=%s, key=%s, newest=%d",
               ( UInt64 )this,
               bucketName,
               key,
               newest);
    ...
    try
    {
        ...
        WsGetRequest request( key, loader, m_storType, newest );
                   ...
                   bool newest )
{
    ...
    get( bucketName, key, &loader, response, newest );
}

void
WsConnection::pendGet( AsyncMan *asyncMan,
                       const char *bucketName,
                       const char *key,
                       void *buffer,
                       size_t size,
                       bool newest )
{
    ...
    LOG_TRACE( "enter pendGet: conn=0x%llx, bucket=%s, key=%s, newest=%d",
               ( UInt64 )this,
               bucketName,
               key,
               newest );
    ...
    try
    {
        ...
        std::unique_ptr< WsGetRequest > request( new WsGetRequest( key,
                                                                   buffer,
                                                                   size,
                                                                   m_storType,
                                                                   newest) );
...
WsConnection::completeGet( WsGetResponse *response )
{
    ...
    std::unique_ptr< WsRequest > request( m_asyncRequest );
    m_asyncRequest = NULL;
...
void
WsConnection::listObjects( const char *bucketName,
                           const char *prefix,
                           const char *marker,
                           ...
                           WsListObjectsResponse *response /* out */ )
{
    ...
    if( m_storType == WsStorType::WST_WALRUS && ( !marker || !*marker ) )
    {
        marker = " ";
    }
...
void
WsConnection::listAllObjects( const char *bucketName,
                              const char *prefix,
                              const char *delimiter,
                              WsObjectEnum *objectEnum,
                              unsigned int maxKeysInBatch )
{
...
WsConnection::pendDel( AsyncMan *asyncMan, const char *bucketName, const char *key )
{
    ...
    try
    {
        ...
        std::unique_ptr< WsDelRequest > request( new WsDelRequest( key ) );
        init( *request, bucketName, key );
                        ...
                        WsInitiateMultipartUploadResponse *response /* out */  )
{
    dbgAssert( bucketName );
    dbgAssert( key );
    dbgAssert( m_storType == WsStorType::WST_S3 or
               m_storType == WsStorType::WST_SWIFT );
...
WsException::WsException( const char *fmt, ... )
{
    ...
    va_list args;
    va_start( args, fmt );
    vsnprintf( msg.data(), m_msg.size() - 1, fmt, args );  // -1 to leave room for \0 on Windows.
    va_end( args );
...
void
WsException::summary(const char* op, const char* key)
{
    std::stringstream ss;
    ss << "The " << op << " operation for " << key << " failed. " << m_msg;
    m_msg = ss.str();
}
...
typedef boost::bimap<WsStorType, std::string> TranslationsMap;

TranslationsMap
init_translations()
{
    const std::vector<TranslationsMap::value_type> initv{
        { WsStorType::WST_S3, "S3" },
        { WsStorType::WST_GCS, "GCS" },
        { WsStorType::WST_WALRUS, "WALRUS" },
        { WsStorType::WST_SWIFT, "SWIFT" }
    };
...
std::ostream&
operator<<(std::ostream& os, const WsStorType t)
{
    const auto it = translations.left.find(t);
    if (it != translations.left.find(t))
    {
        os << it->second;
    }
    else
    {
        dbgAssert(t == "invalid WsStorType");
...
std::istream&
operator>>(std::istream& is, WsStorType& t)
{
    std::string s;
    is >> s;

    const auto it = translations.right.find(s);
