...
typedef std::array<uint8_t, MD5_DIGEST_LENGTH> MD5Digest;
...
enum class WsStorType
{
    WST_S3,         /// Amazon S3
    WST_GCS,        /// Google Cloud Storage
    WST_WALRUS,     /// Eucalyptus Walrus
    WST_SWIFT,      /// OpenStack Swift
};

std::ostream&
operator<<(std::ostream&, const WsStorType);

std::istream&
operator>>(std::istream&, WsStorType&);
...
struct WsConfig
{
    ...
    const char     *accKey = nullptr;

    /// Cloud storage secret key.

    const char     *secKey = nullptr;
    ...
    WsStorType      storType = WsStorType::WST_S3;
    ...
    const char     *sslCertFile = nullptr;

    /// Optional: verify the host against the certificate's subject name
...
struct WsPutRequestUploader
{
    ...
    virtual size_t
    onUpload( void *chunkBuf, size_t chunkSize ) = 0;

    // Intentionally not an enum class as we want the auto conversion to int. Probably.
    enum SeekResult
    {
        Ok = 0,
        Fail = 1,
    ...
    virtual SeekResult
    onSeek( off_t /* off */, SeekWhence /* whence */)
    {
...
struct WsGetResponseLoader
{
    ...
    virtual size_t  onLoad( const void *chunkData,
                            size_t chunkSize,
...
struct WsObject
{
    ...
    WsObject( const char *_key,
              const char *_lastModified,
              const char *_etag,
              size_t _size,
              bool _isDir )
        : key( _key ),
...
struct WsObjectEnum
{
    ...
    virtual bool
    onObject( const WsObject &object ) = 0;
};
...
public:
    ...
    void
    createBucket( const char *bucketName,
    ...
    void
    delBucket( const char *bucketName );
    ...
    void
    listAllBuckets( std::vector< WsBucket > *buckets /* out */ );
    ...
    void
    put( const char *bucketName,
         const char *key,
         const void *data,
         size_t size,
         const MD5Digest* md5_digest = nullptr,
         const char *contentType = nullptr,
         unsigned cacheMaxAge = c_noCacheControl,
         bool makePublic = false,
         bool useSrvEncrypt = false,
         WsPutResponse *response = nullptr /* out */ );
    ...
    void
    listObjects( const char *bucketName,
                 const char *prefix,
                 const char *marker,
    ...
    void
    listAllObjects( const char *bucketName,
                    const char *prefix,
                    const char *delimiter,
                    WsObjectEnum *objectEnum,
    ...
    void
    initiateMultipartUpload( const char *bucketName,
                             const char *key,
                             const char *contentType = NULL,
                             unsigned cacheMaxAge = c_noCacheControl,
                             bool makePublic = false,
    ...
    void
    completePut( WsPutResponse *response = NULL /* out */ );
    ...
    /// and truncation happened.
    void
    completeGet( WsGetResponse *response = NULL /* out */ );
    ...
    void
    setConnectTimeout( long connectTime );

    /// Enables HTTP tracing.

    void
    enableTracing( TraceCallback *traceCallback ) { m_traceCallback = traceCallback; }  // nofail
...
private:
    ...
    void
    init( WsRequest& request,
          const char *bucketName,
          const char *key,
          const char *keySuffix = nullptr,
          const char *contentType = nullptr,
          unsigned cacheMaxAge = c_noCacheControl,
          bool makePublic = false,
          bool useSrvEncrypt = false,
    ...
    void
    del( const char *bucketName, const char *key, const char *keySuffix,
    ...
    // TODO: AR: turn that into a unique_ptr to make ownership clear.
    // Async support.
...
{
protected:
    WsException( const char *fmt, ... ) __attribute__((format(printf, 2, 3)));
public:
    virtual         ~WsException() throw() {}
    virtual const char * what() const throw();

    void
    summary(const char* op, const char* key);

protected:
    std::string m_msg;
};

#define MAKE_WS_EXCEPTION(exn)                  \
    class exn                                   \
        : public WsException                    \
    {                                           \
        public:                                 \
        template<typename... A>                 \
            exn(const char* fmt, A... args)     \
            : WsException(fmt, args...)         \
        {}                                      \
                                                \
        virtual ~exn() throw () = default;      \
    }

MAKE_WS_EXCEPTION(WsCurlException);
MAKE_WS_EXCEPTION(WsStdException);
MAKE_WS_EXCEPTION(WsUnknownException);
MAKE_WS_EXCEPTION(WsXmlParserException);
MAKE_WS_EXCEPTION(WsStatusUnexpectedException);
MAKE_WS_EXCEPTION(WsStatusResourceNotFoundException);
MAKE_WS_EXCEPTION(WsStatusHttpFailureException);
MAKE_WS_EXCEPTION(WsStatusFailureWithDetailsException);
MAKE_WS_EXCEPTION(WsTooManyConnectionsException);

#undef MAKE_WS_EXCEPTION
