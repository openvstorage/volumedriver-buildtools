...
dbgTestWsConnection()
{
    ...
    const char* stor_type = getenv ( "WS_STOR_TYPE" );
    if( stor_type != nullptr )
    {
        config.storType = boost::lexical_cast<WsStorType>(stor_type);
    }
    else if( config.host )
    {
        if( strstr( config.host, ".amazonaws.com" ) )
            config.storType = WsStorType::WST_S3;
        else if( strstr( config.host, ".googleapis.com" ) )
            config.storType = WsStorType::WST_GCS;
        else
            config.storType = WsStorType::WST_WALRUS;
    }
    else
    {
        config.storType = WsStorType::WST_S3;
    ...
    if( config.storType == WsStorType::WST_S3 or
        config.storType == WsStorType::WST_SWIFT )
    {
